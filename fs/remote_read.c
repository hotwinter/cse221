...
float seq_read(void *args) {
    ...
    while(total < size) {
        ...
        total += bytes;
        total_time += (after.tv_sec - prev.tv_sec) * 1000.0 + (after.tv_usec - prev.tv_usec) / 1000.0;
    }
    close(fd);
...
int main(int argc, char **argv) {
    int i;
    unsigned int sizes[] = {KB(16), KB(64), KB(128), KB(256), KB(512), MB(1), MB(2), MB(4)};
    char desc[100];
    ...
    for(i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++) {
        ...
        args.size = sizes[i];
        experiment(desc, seq_read, &args, 10, "ms");
    }
    printf("2.2 Random Readtime\n");
    for(i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++) {
        snprintf(desc, sizeof(desc), "%u: ", sizes[i]);
        args.size = sizes[i];
        experiment(desc, rand_read, &args, 10, "ms");
    }
    disable_counters();
    return 0;
}
